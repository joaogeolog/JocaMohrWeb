...
def plot_3d_block(params):
    """Visualização 3D com órbita travada no centro, mantendo planos e nomes intactos."""
    ...
    # Eixos Anderson (Z vertical no Normal)
    if reg == 'Normal':
    ...
    # Geometria do Plano baseada no Mergulho
    ...
    # Vetores Sn e Tau ortogonais e contidos
    ...
    # RESOLUÇÃO DA ROTAÇÃO: Apenas trava o centro da órbita no (0,0,0)
    fig.update_layout(
        scene=dict(
            xaxis_visible=False, yaxis_visible=False, zaxis_visible=False,
            aspectmode='data', # Mantém o tamanho original de tudo
            camera=dict(
                eye=dict(x=1.5, y=1.5, z=1.5),
                center=dict(x=0, y=0, z=0), # Trava a rotação no centro do cubo
                up=dict(x=0, y=0, z=1)
            )
        ), 
