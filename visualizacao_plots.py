...
def plot_3d_block(params):
    """Visualização 3D envolta em um container com borda CSS para delimitar a área."""
    
    # CSS para criar o contorno da área 3D
    st.markdown("""
        <style>
        .viewport-3d {
    ...
    with st.container():
        # Aplicando a classe CSS via div para delimitar a área
        ...
        # Cubo Wireframe, Bounding Box e Vetores (Lógica mantida)
        b = lim
        ...
        fig.update_layout(scene=dict(xaxis=dict(visible=False, range=[-lim, lim]), yaxis=dict(visible=False, range=[-lim, lim]), zaxis=dict(visible=False, range=[-lim, lim]), 
                                     aspectmode='cube', camera=dict(eye=dict(x=1.2, y=1.2, z=1.2), center=dict(x=0, y=0, z=0))), 
