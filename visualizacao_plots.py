...
def plot_mohr(x_env, y_env, xt_coll, xc_f, yc_f, res_c, xc_o, yc_o, sn_p, tn_p, p_x, p_y, falhou, params):
    """Renderiza o Diagrama de Mohr usando o ângulo com S1 para os cálculos."""
    fig = go.Figure()
    ...
    # Círculos de Mohr (Estável e Teórico)
    m_s = (yc_o < res_c - 1e-3)
    m_f = ~m_s & (yc_o > 0.05)
    ...
    # Histórico da Trajetória de Tensão
...
def plot_3d_block(params):
    ...
    # Eixos por regime (Padrão Anderson)
    if reg == 'Normal':
    ...
    def add_arrow(direction, color, name, magnitude, inward=True):
        ...
        fig.add_trace(go.Scatter3d(x=[start_p[0], end_p[0]], y=[start_p[1], end_p[1]], z=[start_p[2], end_p[2]], 
                                   mode='lines', line=dict(color=color, width=6), showlegend=False))
        fig.add_trace(go.Cone(x=[end_p[0]], y=[end_p[1]], z=[end_p[2]], u=[arrow_d[0]], v=[arrow_d[1]], w=[arrow_d[2]], 
                              colorscale=[[0, color], [1, color]], showscale=False, sizemode="absolute", sizeref=12))
        fig.add_trace(go.Scatter3d(x=[start_p[0]*1.15 if inward else end_p[0]*1.2], 
                                   y=[start_p[1]*1.15 if inward else end_p[1]*1.2], 
