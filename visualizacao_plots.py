...
def plot_3d_block(params):
    with st.container(border=True):
        ...
        s2, rad_m, rad_s1 = (s1 + s3) / 2, np.radians(mergulho), np.radians(ang_s1)
        if reg == 'Normal': n_vec, f_dir, e1, e2, e3 = np.array([0, np.sin(rad_m), np.cos(rad_m)]), np.array([0, np.cos(rad_m), -np.sin(rad_m)]), np.array([0,0,1]), np.array([1,0,0]), np.array([0,1,0])
        elif reg == 'Transcorrente': n_vec, f_dir, e1, e2, e3 = np.array([np.cos(rad_m), np.sin(rad_m), 0]), np.array([-np.sin(rad_m), np.cos(rad_m), 0]), np.array([0,1,0]), np.array([0,0,1]), np.array([1,0,0])
        else: n_vec, f_dir, e1, e2, e3 = np.array([np.sin(rad_m), 0, np.cos(rad_m)]), np.array([np.cos(rad_m), 0, -np.sin(rad_m)]), np.array([1,0,0]), np.array([0,1,0]), np.array([0,0,1])
        fig = go.Figure()
