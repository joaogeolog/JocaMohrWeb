...
def plot_mohr(x_env, y_env, xt_coll, xc_f, yc_f, res_c, xc_o, yc_o, sn_p, tn_p, p_x, p_y, falhou, params):
    ...
    # Envoltórias e Círculos com lógica de colapso
    for x, y, c in [(x_env[x_env<=0], y_env[x_env<=0], 'blue'), (x_env[(x_env>0) & (x_env<=xt_coll)], y_env[(x_env>0) & (x_env<=xt_coll)], 'red'), (x_env[x_env>xt_coll], y_env[x_env>xt_coll], 'green')]:
...
def plot_3d_block(params):
    ang_rad, reg = np.radians(params['ang_s1']), params['regime']
    s1, s3 = params['s1'], params['s3']
    s2 = (s1 + s3) / 2
    # Eixos por regime
    e1, e2, e3 = (([0,0,1],[1,0,0],[0,1,0]) if reg=='Normal' else (([0,1,0],[0,0,1],[1,0,0]) if reg=='Transcorrente' else ([1,0,0],[0,1,0],[0,0,1])))
    ...
    # Plano com triangulação manual
    pts = np.array([-42*e2-42*tau_dir, 42*e2-42*tau_dir, 42*e2+42*tau_dir, -42*e2+42*tau_dir])
    ...
    def arrow(dir, col, name, mag, inv=True):
        d = dir / np.linalg.norm(dir); sc = 0.25
        ...
        fig.add_trace(go.Scatter3d(x=[st_p[0]*1.15 if inv else en_p[0]*1.2], y=[st_p[1]*1.15 if inv else en_p[1]*1.2], z=[st_p[2]*1.15 if inv else en_p[2]*1.2], mode='text', text=[f"<b>{name}</b>"], textfont=dict(color=col, size=13), showlegend=False))

    arrow(e1, "blue", "S1", s1); arrow(e2, "green", "S2", s2); arrow(e3, "red", "S3", s3)
    arrow(norm, "black", "Sn", s1*np.cos(ang_rad)**2 + s3*np.sin(ang_rad)**2, False)
    arrow(tau_dir, "orange", "Tau", abs(s1-s3)/2*np.sin(2*ang_rad), False)
