...
def plot_3d_block(params):
    """Renderiza o bloco 3D com plano contido e vetores completos com nomes."""
    ...
    # 2. PLANO CONTIDO NO CUBO
    # Gerando os 4 cantos do plano limitados pelas bordas do bloco (-40 a 40 em X/Y)
    px = np.array([-40, 40, 40, -40])
    py = np.array([-40, -40, 40, 40])
    if reg == 'Transcorrente':
        # Plano vertical rotacionado em Z
        pz = np.array([-50, -50, 50, 50])
        px_r = px*np.cos(ang_rad) - py*np.sin(ang_rad)
        py_r = px*np.sin(ang_rad) + py*np.cos(ang_rad)
        fig.add_trace(go.Mesh3d(x=px_r, y=py_r, z=pz, color='lightblue', opacity=0.8, showlegend=False))
    else:
        # Plano inclinado rotacionado em Y
        pz = px * np.tan(ang_rad)
        fig.add_trace(go.Mesh3d(x=px, y=py, z=pz, color='lightblue', opacity=0.8, showlegend=False))

    # 3. VETORES COMPLETOS (LINHA + CABEÇA + TEXTO)
    def add_full_arrow(direction, color, name, magnitude):
        scale = 0.2
        d = np.array(direction) / np.linalg.norm(direction)
        start_p, end_p = d * 55, d * (55 + magnitude * scale)
        # Corpo do vetor
        ...
        # Cabeça do vetor (Cone)
        fig.add_trace(go.Cone(x=[end_p[0]], y=[end_p[1]], z=[end_p[2]], u=[d[0]], v=[d[1]], w=[d[2]], colorscale=[[0, color], [1, color]], showscale=False, sizemode="absolute", sizeref=10))
        # Nome do vetor
        fig.add_trace(go.Scatter3d(x=[end_p[0] * 1.2], y=[end_p[1] * 1.2], z=[end_p[2] * 1.2], mode='text', text=[f"<b>{name}</b>"], textfont=dict(color=color, size=12), showlegend=False))

    add_full_arrow(v1, "blue", "S1", s1)
    add_full_arrow(v2, "green", "S2", s2)
    add_full_arrow(v3, "red", "S3", s3)
    ...
    add_full_arrow(norm, "black", "Sn", sn_v)
    if tau_v > 1.0:
        add_full_arrow(tau_dir, "orange", "Tau", tau_v)
