...
def plot_mohr(x_env, y_env, xt_coll, xc_f, yc_f, res_c, xc_o, yc_o, sn_p, tn_p, p_x, p_y, falhou, params):
    """Renderiza o Mohr com Sn e Tau (Efetivos)."""
    with st.container(border=True):
        ...
        st.plotly_chart(fig, use_container_width=True, key="mohr_plot_static")

def plot_3d_block(params):
    """Visualização 3D que mantém a rotação do usuário para TODAS as barras."""
    with st.container(border=True):
        ang_s1 = params['ang_s1']
        theta_rad, mergulho_rad = np.radians(ang_s1), np.radians(90 - ang_s1)
        reg, s1, s3 = params['regime'], params['s1'], params['s3']
        s2, lim = (s1 + s3) / 2, 120 
        ...
        def add_arrow(direction, color, name, magnitude, inward=True):
            ...
            fig.add_trace(go.Scatter3d(x=[(st_p if inward else en_p)[0] + d[0] * off], y=[(st_p if inward else en_p)[1] + d[1] * off], z=[(st_p if inward else en_p)[2] + d[2] * off], mode='text', text=[f"<b>{name}</b>"], textfont=dict(color=color, size=14), showlegend=False))
        ...
        # PERSISTÊNCIA TOTAL
        fig.update_layout(
            uirevision=reg, # Trava a câmera para QUALQUER mudança de magnitude
            scene=dict(
                xaxis=dict(visible=False, range=[-lim, lim]), 
                yaxis=dict(visible=False, range=[-lim, lim]), 
                zaxis=dict(visible=False, range=[-lim, lim]), 
                aspectmode='cube',
                camera=dict(eye=dict(x=1.1, y=1.1, z=1.1)) # Posição inicial
            ), 
            height=500, margin=dict(l=0, r=0, t=0, b=0)
        )
        st.plotly_chart(fig, use_container_width=True, key="block_3d_main")
