...
def plot_mohr(x_env, y_env, xt_coll, xc_f, yc_f, res_c, xc_o, yc_o, sn_p, tn_p, p_x, p_y, falhou, params):
    ...
    # Envoltórias, Círculos e Trajetória (Mantidos)
...
def plot_3d_block(params):
    """Visualização 3D com Contorno da Janela de Visualização."""
    ...
    s2 = (s1 + s3) / 2
    lim = 120 # Define o limite da "caixa" de visualização
    ...
    # 1. CONTORNO DA JANELA DE VISUALIZAÇÃO (Bounding Box)
    # Desenha um cubo cinza claro nas extremidades do range [-lim, lim]
    b = lim
    ...
    for e in box_edges:
        fig.add_trace(go.Scatter3d(x=[box_v[e[0]][0], box_v[e[1]][0]], y=[box_v[e[0]][1], box_v[e[1]][1]], z=[box_v[e[0]][2], box_v[e[1]][2]], 
                                   mode='lines', line=dict(color='rgba(200,200,200,0.5)', width=1, dash='dash'), showlegend=False))

    # 2. CUBO GEOLÓGICO (Wireframe interno)
    ...
    fig.update_layout(
        scene=dict(
            ...
            aspectmode='cube',
            camera=dict(eye=dict(x=1.2, y=1.2, z=1.2), center=dict(x=0, y=0, z=0), up=dict(x=0, y=0, z=1))
        ), 
