...
def plot_3d_block(params):
    ...
    with st.container(border=True):
        ...
        def add_arrow(direction, color, name, magnitude, inward=True):
            ...
            txt_p = (st_p if inward else en_p) + d * off
            fig.add_trace(go.Scatter3d(x=[txt_p[0]], y=[txt_p[1]], z=[txt_p[2]], mode='text', text=[f"<b>{name}</b>"], textfont=dict(color=color, size=14), showlegend=False))
        ...
        fig.update_layout(scene=dict(xaxis=dict(visible=False, range=[-lim, lim]), yaxis=dict(visible=False, range=[-lim, lim]), zaxis=dict(visible=False, range=[-lim, lim]), aspectmode='cube', camera=dict(eye=dict(x=1.1, y=1.1, z=1.1), center=dict(x=0, y=0, z=0))), height=500, margin=dict(l=0, r=0, t=0, b=0))
        st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False})
