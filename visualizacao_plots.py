...
def plot_mohr(x_env, y_env, xt_coll, xc_stable, yc_stable, res_env, xc_fail, yc_fail, sn_p, tn_p, p_x, p_y, falhou, params):
    ...
    with st.container(border=True):
        ...
        fig.add_trace(go.Scatter(x=xc_stable, y=yc_stable, line=dict(color='#1f77b4', width=2.5), name="Estado Estável"))
        fig.add_trace(go.Scatter(x=xc_fail, y=yc_fail, line=dict(color='black', width=1, dash='dash'), showlegend=False))
...
def plot_3d_block(params):
    """Bloco 3D completo: cubos, cones, plano e Tau."""
    with st.container(border=True):
        ...
        def add_arrow(direction, color, name, magnitude, inward=True):
            ...
            fig.add_trace(go.Scatter3d(x=[(sp if inward else ep)[0]+d[0]*off], y=[(sp if inward else ep)[1]+d[1]*off], z=[(sp if inward else ep)[2]+d[2]*off], mode='text', text=[f"<b>{name}</b>"], textfont=dict(color=color, size=14), showlegend=False))
        ...
        fig.update_layout(uirevision=reg, scene=dict(xaxis=dict(visible=False), yaxis=dict(visible=False), zaxis=dict(visible=False), aspectmode='cube'), height=500, margin=dict(l=0, r=0, t=0, b=0))
        st.plotly_chart(fig, use_container_width=True)
