...
def plot_mohr(x_env, y_env, xt_coll, xc_s, yc_s, xc_f, yc_f, env_high, sn_p, tn_p, p_x, p_y, falhou, params):
    ...
    with st.container(border=True):
        ...
        # Plotagem das Envoltórias Técnicas
...
def plot_3d_block(params):
    ...
    with st.container(border=True):
        ...
        f_dir, n_vec = -e1 * np.sin(merg_rad) + e3 * np.cos(merg_rad), e1 * np.cos(merg_rad) + e3 * np.sin(merg_rad)
        
        fig = go.Figure()
        box_v = np.array([[-lim,-lim,-lim], [lim,-lim,-lim], [lim,lim,-lim], [-lim,lim,-lim], [-lim,-lim,lim], [lim,-lim,lim], [lim,lim,lim], [-lim,lim,lim]])
        edges = [(0,1), (1,2), (2,3), (3,0), (4,5), (5,6), (6,7), (7,4), (0,4), (1,5), (2,6), (3,7)]
        for e in edges:
            fig.add_trace(go.Scatter3d(x=[box_v[e[0]][0], box_v[e[1]][0]], y=[box_v[e[0]][1], box_v[e[1]][1]], z=[box_v[e[0]][2], box_v[e[1]][2]], mode='lines', line=dict(color='rgba(200,200,200,0.2)', width=1, dash='dash'), showlegend=False))
