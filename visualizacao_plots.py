...
def plot_mohr(x_env, y_env, xt_coll, xc_s, yc_s, xc_f, yc_f, env_high, sn_p, tn_p, p_x, p_y, falhou, params):
    """Renderiza o Diagrama de Mohr com a envoltória beijando o eixo (0.001)."""
...
def plot_3d_block(params):
    """Visualização 3D COMPLETA com título flutuante e vetores restaurados."""
    with st.container(border=True):
        # Título Overlay
        ...
        # Definição dos eixos por regime de Anderson
        ...
        for e in edges:
            fig.add_trace(go.Scatter3d(x=[box_v[e[0]][0], box_v[e[1]][0]], y=[box_v[e[0]][1], box_v[e[1]][1]], z=[box_v[e[0]][2], box_v[e[1]][2]], 
                                     mode='lines', line=dict(color='rgba(200,200,200,0.2)', width=1, dash='dash'), showlegend=False))
        
        # 2. Bloco central sólido
        ...
        # 4. Função de Setas Restaurada
        def add_arrow(direction, color, name, magnitude, inward=True):
            scale, d = 0.22, direction / (np.linalg.norm(direction) + 1e-9)
            if inward: 
                ep, sp, ar_d, off = d*55, d*(55+magnitude*scale), -d, 15
            else: 
                sp, ep, ar_d, off = np.array([0,0,0]), d*(magnitude*scale+15), d, 12
