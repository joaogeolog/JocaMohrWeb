...
def sync_angles(source_key):
    """Sincroniza Ângulo com S1 e Mergulho: dip = 90 - ang_s1."""
...
def dual_input(label, min_v, max_v, key_p, step=1.0):
    ...
    if base_key not in st.session_state:
        st.session_state[base_key] = st.session_state[s_key] = st.session_state[n_key] = float(DEFAULTS.get(key_p, 30.0 if key_p == 'ang' else 60.0))
...
def render_bottom_interface():
    with st.container(border=True):
        ...
        with c2:
            ...
            dual_input("Coesão (MPa)", 0, 100, 'c'); dual_input("Atrito (°)", 0, 90, 'phi'); dual_input("Colapso (MPa)", 0, 500, 'pc')
        with c3:
