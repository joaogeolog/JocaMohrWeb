...
def reset_section(keys):
    ...
    if 'ang' in keys:
        st.session_state['val_ang'], st.session_state['val_mergulho'] = 30.0, 60.0
        sync_angles('val_ang')
...
def dual_input(label, min_v, max_v, key_p, step=1.0):
    ...
    if base_key not in st.session_state:
        st.session_state[base_key] = st.session_state[s_key] = st.session_state[n_key] = float(DEFAULTS.get(key_p, 30.0))
...
def render_ui():
    with st.container(border=True):
        ...
        with c3:
            ...
            dual_input("Ângulo com S1", 0, 90, 'ang'); dual_input("Mergulho", 0, 90, 'mergulho')
