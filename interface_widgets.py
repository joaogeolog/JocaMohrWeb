...
def sync_angles(source_key):
    regime = st.session_state.get('regime_sel', 'Normal')
    if regime == 'Transcorrente': st.session_state['val_mergulho'] = 90.0
    elif regime == 'Normal':
        if source_key == 'val_ang': st.session_state['val_mergulho'] = 90.0 - st.session_state['val_ang']
        else: st.session_state['val_ang'] = 90.0 - st.session_state['val_mergulho']
    elif regime == 'Reverso':
        if source_key == 'val_ang': st.session_state['val_mergulho'] = st.session_state['val_ang']
        else: st.session_state['val_ang'] = st.session_state['val_mergulho']
...
def dual_input(label, min_v, max_v, key_p, step=1.0, disabled=False):
    ...
    val = st.session_state.get(base_key, float(DEFAULTS.get(key_p, 30.0)))
    c_l, c_s, c_n = st.columns([1, 2, 1])
    c_l.markdown(f"<p style='font-size:0.8em; margin-bottom: -15px;'>{label}</p>", unsafe_allow_html=True)
    c_s.slider(label, float(min_v), float(max_v), val, float(step), key=s_key, on_change=sync_widgets, args=(s_key, n_key, base_key), label_visibility="collapsed", disabled=disabled)
    c_n.number_input(label, float(min_v), float(max_v), val, float(step), key=n_key, on_change=sync_widgets, args=(n_key, s_key, base_key), label_visibility="collapsed", disabled=disabled)
...
def render_ui():
    with st.container(border=True):
        ...
        with c3:
            ...
            st.selectbox("Regime Tectônico", ["Normal", "Transcorrente", "Reverso"], key='regime_sel', on_change=st.rerun)
            is_trans = st.session_state.get('regime_sel') == 'Transcorrente'
            dual_input("Ângulo com S1", 0, 90, 'ang'); dual_input("Mergulho (°)", 0, 90, 'mergulho', disabled=is_trans)
