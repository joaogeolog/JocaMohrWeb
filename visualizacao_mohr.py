...
def render_mohr_plot(x_env, y_env, xt_coll, xcf, ycf, env_high, sn_p, tn_p, p_x, p_y, falhou):
    with st.container(border=True):
        ...
        # 2. Círculo Teórico (Preto Tracejado) - Aparece onde o estado vaza a envoltória
        ...
        # 3. Estado Estável (Azul Claro) - Círculo limitado pela envoltória
        fig.add_trace(go.Scatter(
            x=xcf, y=ycf, 
        ...
        # 4. Envoltórias (Tração, Cisalhamento, Colapso)
        ...
        # 5. Ponto do Plano (Círculo Amarelo)
        fig.add_trace(go.Scatter(
            x=[sn_p], y=[tn_p], 
            mode='markers', 
            marker=dict(
