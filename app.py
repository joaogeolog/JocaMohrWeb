...
st.markdown("<style>header {visibility: hidden;} footer {visibility: hidden;} .main .block-container {padding-top: 0rem !important; margin-top: -45px !important;}</style>", unsafe_allow_html=True)

if 'val_s1' not in st.session_state:
    for k, v in ui.DEFAULTS.items(): 
        st.session_state[f'val_{k}'] = st.session_state[f'slide_{k}'] = st.session_state[f'num_{k}'] = float(v)
    st.session_state['regime_sel'] = 'Normal'
    ui.sync_angles('val_ang') 
    st.session_state.path_x, st.session_state.path_y = [], []
    st.session_state.ponto = {'sn': 0.0, 'tn': 0.0}
...
sn, tn, fail = eng.calcular_ponto_com_trava(s1_e, s3_e, p['ang_s1'], xe, ye, p['ts'], p['pc'], st.session_state.get('ponto', {'sn': 0.0, 'tn': 0.0}))
...
with c_mohr: vmohr.render_mohr_plot(xe, ye, xt, xcf, ycf, m_high, sn, tn, xco, yco, fail)
